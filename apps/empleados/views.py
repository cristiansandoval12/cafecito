...
def EmpleadoInactivar(request, id):
    template_name="empleados/empleados_inactivar.html"
    contexto={}
    prv=empleados.objects.filter(pk=id).first()
    
    if not prv:
        return HttpResponse("Empleado no existe" + str(id))
